...
function badgeTone(value: string) {
  ...
  return "bg-[#EEF6FF] text-[#002C5F] ring-blue-100";
}
...
function TextInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className="h-9 rounded-md border border-blue-100 bg-white px-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#0072CE] focus:ring-2 focus:ring-blue-100" />;
}

function SelectInput(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return <select {...props} className="h-9 rounded-md border border-blue-100 bg-white px-2.5 text-sm text-slate-900 outline-none transition focus:border-[#0072CE] focus:ring-2 focus:ring-blue-100" />;
}
...
function SectionTitle({ eyebrow, title, action }: { eyebrow: string; title: string; action?: React.ReactNode }) {
  return (
    <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
      <div>
        <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#0072CE]">{eyebrow}</p>
        <h2 className="mt-0.5 text-lg font-semibold text-[#002C5F]">{title}</h2>
      </div>
      {action}
    </div>
  );
...
function EmptyState({ title, detail }: { title: string; detail: string }) {
  return (
    <div className="rounded-lg border border-dashed border-blue-200 bg-[#EEF6FF] px-5 py-6 text-center">
  ...
  return (
    <main className="min-h-screen bg-[#F6F9FC] text-slate-900">
      <div className="mx-auto grid w-full max-w-[1400px] gap-3 px-3 py-3 lg:px-5">
        <header className="sticky top-0 z-20 rounded-xl border border-blue-100 bg-white/95 px-3 py-3 shadow-[0_8px_30px_rgba(0,44,95,0.08)] backdrop-blur">
          <div className="grid gap-3 lg:grid-cols-[250px_minmax(260px,1fr)_auto] lg:items-center">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#0072CE]">Connected fleet</p>
              <h1 className="text-xl font-bold tracking-tight text-[#002C5F]">Vehicle Command Center</h1>
            </div>
            ...
            <button onClick={exportCsv} className="rounded-md bg-[#002C5F] px-3 py-2 text-sm font-semibold text-white hover:bg-[#001E42]">Export CSV</button>
          </div>
          <div className="mt-3">
            <nav className="flex gap-1 overflow-x-auto rounded-lg border border-blue-100 bg-[#EEF6FF] p-1">
              {views.map((item) => <button key={item} onClick={() => setView(item)} className={`shrink-0 rounded-md px-2.5 py-1.5 text-xs font-semibold ${view === item ? "bg-[#002C5F] text-white shadow-sm" : "text-[#002C5F] hover:bg-white"}`}>{navLabel(item)}</button>)}
            </nav>
          </div>
          <div className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-2 lg:grid-cols-4">
            <span className="rounded-md border border-blue-100 bg-[#EEF6FF] px-3 py-2"><strong className="text-[#002C5F]">Status:</strong> {activeVehicle.status || "Active"}</span>
            <span className="rounded-md border border-blue-100 bg-[#EEF6FF] px-3 py-2"><strong className="text-[#002C5F]">Driver:</strong> {activeVehicle.assignedDriver || "Unassigned"}</span>
            <span className="rounded-md border border-blue-100 bg-[#EEF6FF] px-3 py-2"><strong className="text-[#002C5F]">Odometer:</strong> {km.format(activeVehicle.currentOdometer)} km</span>
            <span className="rounded-md border border-blue-100 bg-[#EEF6FF] px-3 py-2"><strong className="text-[#002C5F]">Next service:</strong> {headerNextMaintenance ? `${headerNextMaintenance.item} at ${km.format(headerNextMaintenance.nextDueKm)} km` : "No open service"}</span>
          </div>
        </header>
          ...
          <aside className="grid gap-3 self-start">
            <Panel>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button onClick={() => setEditingVehicle(activeVehicle)} className="rounded-md border border-[#0072CE] bg-white px-3 py-2 text-sm font-semibold text-[#002C5F] hover:bg-[#EEF6FF]">Edit</button>
            ...
            <Panel>
              <details>
                <summary className="cursor-pointer list-none rounded-lg border border-blue-100 bg-[#EEF6FF] px-3 py-2 text-sm font-semibold text-[#002C5F]">Add vehicle profile</summary>
              <form onSubmit={addVehicle} className="mt-3 grid gap-3">
                ...
                <Field label="Current odometer"><TextInput name="currentOdometer" type="number" placeholder="0" /></Field>
                  <button className="rounded-md bg-[#002C5F] px-4 py-2 text-sm font-semibold text-white hover:bg-[#001E42]">Create vehicle</button>
              </form>
              </details>
            </Panel>
...
function Panel({ children }: { children: React.ReactNode }) {
  return <section className="self-start rounded-xl border border-blue-100 bg-white p-3 shadow-[0_10px_35px_rgba(0,44,95,0.06)]">{children}</section>;
}

function MetricStrip({ items }: { items: Array<{ label: string; value: string; detail: string }> }) {
  return (
    <section className="grid overflow-hidden rounded-xl border border-blue-100 bg-white shadow-[0_10px_35px_rgba(0,44,95,0.06)] sm:grid-cols-2 xl:grid-cols-5">
      {items.map((item) => (
        <article key={item.label} className="border-b border-blue-50 p-3 sm:border-r xl:border-b-0 last:border-r-0">
          <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#0072CE]">{item.label}</p>
          <strong className="mt-1 block text-xl font-semibold text-[#002C5F]">{item.value}</strong>
...
function Kpi({ label, value, detail, tone = "slate" }: { label: string; value: string; detail: string; tone?: "slate" | "amber" | "rose" | "emerald" }) {
  const colors = { slate: "border-l-[#002C5F]", amber: "border-l-amber-400", rose: "border-l-red-500", emerald: "border-l-emerald-500" };
  return <article className={`min-h-24 rounded-xl border border-l-4 border-blue-100 bg-white p-3 shadow-[0_1px_2px_rgba(0,44,95,0.04)] ${colors[tone]}`}><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#0072CE]">{label}</p><strong className="mt-2 block text-2xl font-bold text-[#002C5F]">{value}</strong><span className="mt-1 block text-sm leading-5 text-slate-500">{detail}</span></article>;
}
    ...
    <>
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_360px]">
        <Panel>
          <SectionTitle eyebrow="Command center" title="Needs attention" action={<div className="flex flex-wrap gap-2">{[
            ...
            ["maintenance", "Service"],
            ["document", "Document"],
          ].map(([type, label]) => <button key={type} onClick={() => setDrawer(type)} className="rounded-md border border-[#0072CE] bg-white px-2.5 py-1.5 text-xs font-semibold text-[#002C5F] hover:bg-[#EEF6FF]">{label}</button>)}</div>} />
          <div className="grid gap-2">
            {attentionItems.length ? attentionItems.map((item: any) => <button key={`${item.title}-${item.detail}`} onClick={() => setView(item.view)} className="grid gap-1 rounded-lg border border-blue-100 bg-white p-3 text-left hover:border-[#0072CE] hover:bg-[#EEF6FF]"><div className="flex items-center justify-between gap-3"><strong className="text-sm text-[#002C5F]">{item.title}</strong><Badge tone={item.priority === "Critical" ? "bg-red-50 text-red-700 ring-red-200" : item.priority === "Review" ? "bg-[#EEF6FF] text-[#002C5F] ring-blue-100" : "bg-amber-50 text-amber-700 ring-amber-200"}>{item.priority}</Badge></div><span className="text-sm text-slate-500">{item.detail}</span></button>) : <EmptyState title="No open work queue" detail="The fleet has no urgent inspection, service, or cost review items." />}
          </div>
        </Panel>
        <Panel>
          <SectionTitle eyebrow="What changed" title="This month" />
          <div className="grid gap-2">
            {insights.map((item) => <p key={item} className="rounded-lg border border-blue-100 bg-[#EEF6FF] p-3 text-sm leading-5 text-slate-700">{item}</p>)}
          </div>
        </Panel>
      </div>
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_330px]">
        <Panel>
          ...
          {analytics.monthlyByType.length ? <div className="grid min-h-56 grid-cols-2 items-end gap-3 md:grid-cols-5 lg:grid-cols-10">{analytics.monthlyByType.map((row: any) => <div key={row.type} className="grid h-full content-end gap-2 text-center"><div className="rounded-t-md bg-[#0072CE] shadow-[0_8px_18px_rgba(0,114,206,0.18)]" style={{ height: `${Math.max((row.total / maxMonthly) * 150, 14)}px` }} /><strong className="text-xs text-[#002C5F]">{eur.format(row.total)}</strong><span className="text-[11px] text-slate-500">{row.type}</span></div>)}</div> : <EmptyState title="No spend this month" detail="Add fuel, service, insurance, tolls, or other ledger entries." />}
        </Panel>
        <Panel>
          <SectionTitle eyebrow="Next 30 days" title="Upcoming work" />
          <div className="grid gap-2">
            {next30.length ? next30.map((item: any) => <button key={`${item.label}-${item.date}`} onClick={() => setView(item.view)} className="rounded-lg border border-blue-100 bg-white p-3 text-left hover:border-[#0072CE] hover:bg-[#EEF6FF]"><strong className="text-sm text-[#002C5F]">{item.label}</strong><p className="text-sm text-slate-500">{item.date} · {item.detail}</p></button>) : <EmptyState title="Nothing due soon" detail="No selected-vehicle tasks are due inside 30 days." />}
          </div>
        </Panel>
      </div>
    </>
...
function SummaryCard({ label, value, detail }: { label: string; value: string; detail: string }) {
  return <article className="rounded-lg border border-blue-100 bg-[#EEF6FF] p-3"><p className="text-[11px] font-bold uppercase tracking-[0.08em] text-[#0072CE]">{label}</p><strong className="mt-1.5 block text-xl font-bold text-[#002C5F]">{value}</strong><span className="text-sm leading-5 text-slate-500">{detail}</span></article>;
}
  ...
  return (
    <Panel>
      ...
      <div className="grid gap-2 md:grid-cols-5">
        {items.map((item) => (
          <article key={item.label} className="rounded-lg border border-blue-100 bg-[#EEF6FF] p-3">
            <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-[#0072CE]">{item.label}</p>
            <strong className="mt-2 block text-lg font-semibold text-[#002C5F]">{item.value}</strong>
...
function Table({ headers, children }: { headers: string[]; children: React.ReactNode }) {
  return <div className="overflow-x-auto rounded-lg border border-blue-100"><table className="w-full min-w-[900px] border-collapse bg-white text-left text-sm text-slate-700"><thead className="bg-[#EEF6FF] text-[11px] uppercase tracking-[0.08em] text-[#002C5F]"><tr>{headers.map((header) => <th key={header} className="sticky top-0 border-b border-blue-100 px-3 py-2.5 font-bold">{header}</th>)}</tr></thead><tbody className="divide-y divide-blue-50 tabular-nums [&_tr:hover]:bg-[#EEF6FF] [&_td]:px-3 [&_td]:py-2.5">{children}</tbody></table></div>;
}

function Actions({ onEdit, onArchive, onDelete }: { onEdit?: () => void; onArchive: () => void; onDelete: () => void }) {
  return <div className="flex flex-wrap gap-2">{onEdit ? <button onClick={onEdit} className="rounded-md border border-[#0072CE] bg-white px-2.5 py-1.5 text-xs font-semibold text-[#002C5F]">Edit</button> : null}<button onClick={onArchive} className="rounded-md border border-blue-100 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700">Archive</button><button onClick={onDelete} className="rounded-md border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs font-semibold text-red-700">Delete</button></div>;
}

function PrimaryButton({ children, onClick }: { children: React.ReactNode; onClick?: () => void }) {
  return <button onClick={onClick} className="rounded-md bg-[#002C5F] px-3 py-2 text-sm font-semibold text-white hover:bg-[#001E42]">{children}</button>;
}
...
function RecordDrawer({ type, activeVehicle, onClose, forms }: { type: Exclude<DrawerType, null>; activeVehicle: Vehicle; onClose: () => void; forms: Record<Exclude<DrawerType, null>, (event: FormEvent<HTMLFormElement>) => void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/35">
      <aside className="h-full w-full max-w-xl overflow-y-auto border-l border-blue-100 bg-white p-5 shadow-2xl">
        <SectionTitle eyebrow="Record drawer" title={titles[type]} action={<button onClick={onClose} className="rounded-md border border-blue-100 bg-white px-3 py-1.5 text-sm font-semibold text-[#002C5F] hover:bg-[#EEF6FF]">Close</button>} />
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4">
      <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-2xl">
        <h2 className="text-lg font-semibold text-[#002C5F]">{title}</h2>
        <p className="mt-2 text-sm leading-6 text-slate-600">{detail}</p>
        <div className="mt-5 flex justify-end gap-2">
          <button onClick={onClose} className="rounded-md border border-blue-100 px-3 py-2 text-sm font-semibold text-slate-700">Cancel</button>
